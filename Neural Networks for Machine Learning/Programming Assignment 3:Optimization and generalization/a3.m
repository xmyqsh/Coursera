...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]);   % (10, num) = (10, num) - repmat((1, num), [10, 1])
  ...
  ret.hid_to_class = d_loss_by_d_class_input * hid_output';                             % (10, h) = (10, num) * (h, num)'
  ret.hid_to_class = ret.hid_to_class ./ num_cases + wd_coefficient * model.hid_to_class;

  d_loss_by_d_hid_output = model.hid_to_class' * d_loss_by_d_class_input;               % (h, num) = (10, h)' * (10, num)
  d_loss_by_d_hid_input = d_loss_by_d_hid_output .* hid_output .* (1 - hid_output);     % (h, num) = (h, num) .* (h, num) .* (h, num)
  ret.input_to_hid = d_loss_by_d_hid_input * data.inputs';                              % (h, 256) = (h, num) * (256, num)'
