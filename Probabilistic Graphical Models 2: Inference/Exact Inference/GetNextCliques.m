...
N = length(messages);
messaged = zeros(size(messages));
for m = 1 : N
  for n = 1 : N
    messaged(m, n) = ~isempty(messages(m, n).var);
  end
end

for m = 1 : N
  for n = 1 : N
    if P.edges(m, n) == 0 || messaged(m, n) == 1
      continue;
    end
    toMthEdges = P.edges(:, m);
    toMthMessages = messaged(:, m);
    toMthEdges(n) = 0;
    toMthMessages(n) = 0;
    if sum(toMthEdges == toMthMessages) == N
      i = m;
      j = n;
      return;
    end
